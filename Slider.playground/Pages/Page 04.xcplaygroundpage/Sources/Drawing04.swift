...
public class Drawing04 : Drawing{
    let guide = GuideLine()
    let house = Target(imageNamed: "House.png")
    let school:SKSpriteNode = SKSpriteNode(imageNamed: "School.png")
    ...
    open override func handleTouchUp(atPoint pos: CGPoint) -> GameState? {
        if let nextState = super.handleTouchUp(atPoint: pos){
            if let _ = nextState as? Playing{
                return Playing04(Scene:Scene, pos: CGPoint(x: Scene.size.width/10 * 6.7 + 15, y: Scene.size.height/10 * 2.9 + 90))
            }
            return nextState
        }
		else{
